...
<main class="ui-page">
    ...
    @else
        ...
        @foreach($transfers as $transfer)
            <tr><td><a href="{{ route('warehouse.transfers.show', $transfer) }}">{{ $transfer->nomor }}</a>@if($transfer->retur_dari_id)<div class="ui-muted">Retur dari #{{ $transfer->retur_dari_id }}</div>@endif</td><td>{{ $transfer->tanggal?->format('d M Y') }}</td><td>{{ $transfer->origin->kode }} → {{ $transfer->destination->kode }}</td><td>{{ $transfer->items->count() }} item</td><td><span class="ui-badge ui-badge--{{ $transfer->status === 'terbit' ? 'pending' : ($transfer->status === 'diterima' ? 'done' : 'cancelled') }}">{{ ucfirst($transfer->status) }}</span></td><td class="ui-inline"><a class="ui-button ui-button--muted" href="{{ route('warehouse.transfers.show', $transfer) }}">Detail</a><a class="ui-button ui-button--muted" href="{{ route('warehouse.transfers.print', $transfer) }}" target="_blank" rel="noopener noreferrer">Cetak</a></td></tr>
        @endforeach
