...
Glosarium istilah domain. **Setiap nama yang menunjuk konsep di glosarium ini memakai ejaan glosarium, di lapisan mana pun** — kolom database, nama kelas dan relasi, kunci payload, nama field HTML dan atribut `data-*`, judul issue, dan teks UI. Nama yang menunjuk konsep **di luar** glosarium tidak terjerat sama sekali: path `rclone`, API framework seperti `getRawOriginal()`, istilah teknis yang memang bukan istilah domain. Aturan ini berlaku ke depan dan tidak ditagih surut; nama lama yang melanggar jadi tiket sendiri saat ditemukan. Lihat `docs/adr/0027-glosarium-mengikat-nama-yang-menunjuk-konsepnya.md`, [#134](https://github.com/thcjatim-bit/project-monitoring-system/issues/134), dan [#165](https://github.com/thcjatim-bit/project-monitoring-system/issues/165).
...
- **SN** — Serial Number material tertentu; dilacak sampai keluar gudang dan untuk project mana. Satu SN adalah tepat satu pcs dan tidak terbagi.
- **Qty utuh** — qty material selalu bilangan bulat, untuk ketiga jenis material dan semua satuan: satu SN satu pcs, material biasa per unit utuh, kabel per meter utuh dengan minimum 1 meter. Setengah unit bukan benda dan bukan angka yang sah. Lihat `docs/adr/0025-qty-material-bilangan-bulat.md`.
...
- **Surat Jalan** — dokumen perpindahan material antar gudang, nomor `SJ-YYMM-NNNN`, wajib bisa dicetak. Dipakai untuk semua arah (THC↔mitra, THC↔THC); berawal dari Request Material atau dari input langsung petugas THC. Mitra Surat Jalan ditentukan gudang asal, dan jatuh ke gudang tujuan bila asal milik THC; Project yang disebut Surat Jalan harus milik Mitra itu. Siklusnya `terbit` → `diterima`, atau `terbit` → `dibatalkan`.
- **Gudang asal & Gudang tujuan** — dua ujung perpindahan satu Surat Jalan, tersimpan di `warehouse_asal_id` dan `warehouse_tujuan_id`. Gudang asal yang menentukan Mitra Surat Jalan; gudang tujuan mengambil alih bila asal milik THC. Dieja `asal`/`tujuan` di mana pun namanya muncul — relasi Eloquent, kunci payload form, atribut `data-*`. Kata `origin`/`destination` di jalur sinkronisasi foto adalah konsep lain (path sumber dan tujuan `rclone`) dan tidak terjerat aturan ini.
- **Sumber baris Surat Jalan** — asal-usul satu baris pada form Terbitkan Surat Jalan: `prefill` bila baris itu lahir dari Request Material yang dirujuk, `operator` bila petugas mengetiknya sendiri. Membawa niat petugas, yang tidak dapat direkonstruksi server — baris `operator` boleh saja membawa material yang kebetulan ada di daftar request. Menentukan apakah material baris itu terkunci. Kata `asal` tidak dipakai untuk arti ini; di seluruh Surat Jalan `asal` berarti gudang asal.
- **Baris Menyimpang** — baris Surat Jalan yang membawa material di luar daftar Request Material yang dirujuk, atau qty melebihi **sisa** permintaan (`diminta − sudah terkirim`). Daftar request adalah *prefill*, bukan plafon: penyimpangan diperbolehkan, tetapi wajib beralasan per baris dan tercatat di Linimasa Gabungan. Mengirim kurang dari sisa bukan penyimpangan — itu kirim bertahap. Status menyimpang **dibekukan saat Surat Jalan terbit** dan tidak pernah dihitung ulang: sisa bergerak seiring Surat Jalan lain terbit, sedangkan dokumen yang sudah ditandatangani tidak boleh berubah arti. Lihat `docs/adr/0024-daftar-request-material-prefill-bukan-plafon.md`.
...
- **Drum** — satu roll kabel fisik dengan `panjang_awal` dan `sisa`. Potongan yang keluar jadi **drum turunan** ber-ID `DRM-00042-1`, punya baris sendiri, tidak pernah digabung balik ke induk. Surat Jalan boleh **memotong Drum saat terbit**: qty di bawah sisa melahirkan turunan yang berangkat sementara induknya tinggal di gudang asal, sedangkan qty tepat sama dengan sisa mengirim Drum itu sendiri tanpa membuat turunan. Qty di atas sisa ditolak, dan satu Drum hanya boleh muncul sekali per Surat Jalan.
