...
- **Rekon Material** — pencocokan THC ↔ Mitra atas Material yang keluar Warehouse untuk satu Project, dicatat di `project_rekons` bernomor `REK-YYMM-NNNN`. Satu Project bisa punya beberapa rekon: rekon susulan **mengoreksi** rekon sebelumnya (`koreksi_dari_id`, pola sama dengan koreksi buku transaksi), bukan menggantikannya. Isi per baris Material: keluar Warehouse, terpasang, sisa di Project, dikembalikan, hilang/rusak. THC yang approve. Sisa dikembalikan ke **Warehouse Mitra asal** (bukan ditarik ke Warehouse THC). Lihat `docs/adr/0013-alur-pemakaian-material-dan-rekon.md`.
...
- **Antrean Keputusan** — kumpulan entitas sumber yang sedang menunggu keputusan dari aktor yang berwenang, misalnya Request Material atau Pemakaian Material berstatus `diajukan`. Ini bukan entitas transaksi baru dan keputusan tetap dilakukan lewat pemilik alur asal.
- **Dashboard Mitra** — ringkasan baca untuk satu Mitra yang menggabungkan konteks Project, progres jasa, Material, Pemakaian Material, dan Rekon Material yang boleh dilihat user tersebut. Dashboard Mitra bukan pengganti halaman sumber dan tidak menjadi tempat mutasi.
