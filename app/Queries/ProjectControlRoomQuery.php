...
class ProjectControlRoomQuery
{
    ...
    public function for(Project $project, CarbonInterface|string|null $asOf = null, ?User $viewer = null): array
    {
        ...
        $installationDrums = $canReportInstallation
            ? Drum::query()
                ...
                ->get()
            : new Collection;
        $installationHistory = $canReadInstallation
            ? MaterialTransaksi::query()
                ->where('project_id', $project->id)
                ->where('lokasi_tipe', 'terpasang')
                ->where('jenis_transaksi', 'terpasang')
                ->where('qty_delta', '>', 0)
                ->with(['material.unit', 'actor'])
                ->latest('id')
                ->get()
            : new Collection;
        ...
        return [
            ...
            'canReadInstallation' => $canReadInstallation,
    ...
    public function errorState(Project $project): array
    {
        ...
        return [
            ...
            'installationHistory' => new Collection,
